...
const Core::ParameterEnum Connection::paramConnectionType_("type",
		"plain-connection, unit-mapping-connection, weight-connection, convolutional-connection, valid-convolutional-connection",
...
/*
 * UnitMappingConnection
 */

const Core::ParameterInt UnitMappingConnection::paramSourceUnitFrom_("source-unit-from", 0, "neural-network.connection");

const Core::ParameterInt UnitMappingConnection::paramSourceUnitTo_("source-unit-to", 0, "neural-network.connection");

const Core::ParameterInt UnitMappingConnection::paramDestinationUnitFrom_("destination-unit-from", 0, "neural-network.connection");

const Core::ParameterInt UnitMappingConnection::paramDestinationUnitTo_("destination-unit-to", 0, "neural-network.connection");

UnitMappingConnection::UnitMappingConnection(const char* name, BaseLayer* source, BaseLayer* dest, u32 sourcePort, u32 destPort, bool isRecurrent, ConnectionType type) :
		Precursor(name, source, dest, sourcePort, destPort, isRecurrent, type),
		sourceUnitFrom_(Core::Configuration::config(paramSourceUnitFrom_, prefix_)),
		sourceUnitTo_(Core::Configuration::config(paramSourceUnitTo_, prefix_)),
		destUnitFrom_(Core::Configuration::config(paramDestinationUnitFrom_, prefix_)),
		destUnitTo_(Core::Configuration::config(paramDestinationUnitTo_, prefix_))
{
	require_le(sourceUnitFrom_, sourceUnitTo_);
	require_le(destUnitFrom_, destUnitTo_);
	require_eq(sourceUnitTo_ - sourceUnitFrom_ + 1, destUnitTo_ - destUnitFrom_ + 1);
}

void UnitMappingConnection::initialize() {
	tmpMatrix_.initComputation();
	Precursor::initialize();
}

void UnitMappingConnection::_forwardWeightMultiplication(const Matrix& source, Matrix& dest) {
	tmpMatrix_.resize(dest.nRows(), dest.nColumns());
	tmpMatrix_.setToZero();
	require_lt(sourceUnitTo_, source.nRows());
	require_lt(destUnitTo_, dest.nRows());
	require_eq(source.nColumns(), dest.nColumns());
	tmpMatrix_.copyBlockFromMatrix(source,
			sourceUnitFrom_, 0, // source row and column
			destUnitFrom_, 0, // destination row and column
			sourceUnitTo_ - sourceUnitFrom_ + 1, dest.nColumns() // block size
			);
	dest.add(tmpMatrix_);
}

void UnitMappingConnection::_backpropagateWeights(const Matrix& source, Matrix& dest) {
	tmpMatrix_.resize(dest.nRows(), dest.nColumns());
	tmpMatrix_.setToZero();
	require_lt(sourceUnitTo_, dest.nRows());
	require_lt(destUnitTo_, source.nRows());
	require_eq(source.nColumns(), dest.nColumns());
	tmpMatrix_.copyBlockFromMatrix(source,
			destUnitFrom_, 0, // source row and column
			sourceUnitFrom_, 0, // destination row and column
			sourceUnitTo_ - sourceUnitFrom_ + 1, dest.nColumns() // block size
			);
	dest.add(tmpMatrix_);
}
...
Connection* Connection::createConnection(const char* name, BaseLayer* source, BaseLayer* dest, u32 sourcePort, u32 destPort, bool isRecurrent) {
	...
	case weightConnection:
		...
		break;
	case unitMappingConnection:
		connection = new UnitMappingConnection(name, source, dest, sourcePort, destPort, isRecurrent, type);
		break;
