...
void addSummedColumnsChannelWise(const CudaMatrix<T>& matrix, const u32 channels, const T scale = 1.0);
...
template<typename T>
void CudaVector<T>::resize(u32 newSize, bool reallocate) {
...
T CudaVector<T>::max() const {
	...
	if (gpuMode_) {
		...
		Cuda::alloc(resultDev, 1);
		Cuda::argMax(d_elem_, nRows_, 1, resultDev);
		Cuda::copyFromGpu(&index, resultDev, 1);
		Cuda::free(resultDev);
		return get(index);
	} else {
...
template<typename T>
void CudaVector<T>::addSummedColumnsChannelWise(const CudaMatrix<T>& matrix, const u32 channels, const T scale)
{
	require(isComputing_);
	require(matrix.isComputing_);
	require_eq(matrix.nRows() % channels, 0);
	require_eq(nRows_, channels);

	if(matrix.nRows() == channels) {
		addSummedColumns(matrix, scale);
	} else if(gpuMode_) {
		Cuda::addSummedColumnsChannelWise(d_elem_, matrix.d_elem_, channels,
				matrix.nRows(), matrix.nColumns(), scale);
	} else {
		Precursor::addSummedColumnsChannelWise(matrix, channels, scale);
	}
}
template<typename T>
...
template<typename T>
std::string CudaVector<T>::toString(bool transpose) const {
	require(!isComputing_);
