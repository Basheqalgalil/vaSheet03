...
void Stip::opticalFlow(const Video& video, Video& flow) {
	flow.clear();
...
void Stip::createHistogram(const Video& grad, const Video& flow, const InterestPoint& p, Float sigma, Float tau) {
	...
	for (u32 t = std::max(0, (s32)p.t - tpSize); t < std::min((u32)grad.size(), p.t + tpSize); t++) {
		MemoryAccessor dataHog(grad.at(t).rows, grad.at(t).cols, (Float*)grad.at(t).data);
		MemoryAccessor dataHof(flow.at(t).rows, flow.at(t).cols, (Float*)flow.at(t).data);
		for (u32 x = std::max(0, (s32)p.x - spSize); x < std::min((u32)grad.at(t).rows, p.x + spSize); x++) {
			for (u32 y = std::max(0, (s32)p.y - spSize); y < std::min((u32)grad.at(t).rows, p.y + spSize); y++) {
				...
				u32 binIdx = u32(dataHog(x,y) / 45.1) % 4; // 45° leads to 8 bins, % 4: mirror to make sign independent
				histograms.at(histIdx).at(binIdx) += 1.0;
				binIdx = u32(dataHof(x,y) / 45.1) % 4; // 45° leads to 8 bins, % 4: mirror to make sign independent
				histograms.at(histIdx + 3*3*2).at(binIdx) += 1.0;
			}
		}
	}
...
void Stip::extractFeatures(const std::string& filename) {
	...
	for (u32 sp = 0; sp < spatialScales_.size(); sp++) {
		for (u32 tp = 0; tp < temporalScales_.size(); tp++) {
			...
			Video grad; // oriented gradients
			for (u32 t = 0; t < Lx.size(); t++) {
				cv::Mat magnitude, angle;
				cv::cartToPolar(Lx.at(t), Ly.at(t), magnitude, angle, true);
				grad.push_back(angle);
			}
			/* compute hog/hof descriptors */
			for (u32 i = 0; i < interestPoints_.size(); i++) {
				createHistogram(grad, flow, interestPoints_.at(i) , spatialScales_.at(sp), temporalScales_.at(tp));
			}
		}
	}
}
